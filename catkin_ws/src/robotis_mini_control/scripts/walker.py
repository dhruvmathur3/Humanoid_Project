...
class Walker:
    ...
    def _do_walk(self):
        ...
        while not rospy.is_shutdown() and (self.walking or i<n or self.is_displacing()):
            ...
            """SHAOYU: Config for inital lunge is below. Couldn't figure out how to only make it for inital pose without messing with walking dynamics.
            If you can modify the dynamics to do the lunge step then this position can be moved to the init"""
            q = self.robotis_mini_ci.get_q()
            # print(q) = {'l_ankle_joint': -0.180333122616811, 'l_biceps_joint': 0.0065833793878766755, 'l_elbow_joint': 0.0019420239226439762, 'l_foot_joint': -0.008631522656934898, 'l_hip_joint': -0.0031592425562481097, 'l_knee_joint': 0.7181374554697539, 'l_shoulder_joint': 0.00027758580840409053, 'l_thigh_joint': -0.7016925014464581, 'r_ankle_joint': -0.19299919751523653, 'r_biceps_joint': 0.0065869914035960875, 'r_elbow_joint': 0.0019115820328847732, 'r_foot_joint': -0.007614713592440481, 'r_hip_joint': -0.001811670298975443, 'r_knee_joint': 0.7175071049052848, 'r_shoulder_joint': 0.00031156670689291843, 'r_thigh_joint': -0.6899415977544354}
            q['r_hip_joint'] = 0
            q['r_thigh_joint'] = -math.pi/2
            q['r_knee_joint'] = math.pi/2
            q['r_ankle_joint'] = 0
            q['r_foot_joint'] = 0
            q['l_hip_joint'] = 0
            q['l_thigh_joint'] = 0
            q['l_knee_joint'] = math.pi/2
            q['l_ankle_joint'] = -math.pi/2
            q['l_foot_joint'] = 0
            q['r_shoulder_joint'] = math.pi/2
            q['r_biceps_joint'] = math.pi/2
            q['r_elbow_joint'] = 0
            q['l_shoulder_joint'] = 0
            q['l_biceps_joint'] = 0
            q['l_elbow_joint'] = math.pi/2
            self.robotis_mini_ci.set_qd(q)
            i+=1
            if i>n:
                i=0
                p=not p
...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        #for jn in walker.robotis_mini_ci.q_names:
        #    walker.pubs[jn].publish(0.0)
        time.sleep(1)
